...
export interface SampledSource extends SampleableSource {
  weight: number;
}
...
function freshnessFactor(s: SampleableSource, cfg: SampleConfig): number {
  ...
  return now - last <= cfg.freshnessWindowDays * 86400000
    ? cfg.freshnessFactorActive
    : cfg.freshnessFactorStale;
}
