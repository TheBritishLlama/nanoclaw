...
function trialStatsForDomain(
  ...
  const row = db
    ...
    WHERE q.source_url LIKE ? AND r.rated_at >= ?`,
    )
    .get(`%${domain}%`, sinceIso) as { c: number; a: number | null };
...
function setStatus(
  ...
  db.prepare(
    `UPDATE stack_candidate_sources
              ...
              WHERE domain = ?`,
  ).run(status, status, atIso, domain);
}
...
export function runPromotionPass(
  ...
  const sinceIso = new Date(
    now.getTime() - cfg.trialWindowDays * 86400000,
  ).toISOString();
  ...
  for (const c of listCandidatesByStatus(db, 'candidate')) {
    ...
    db.prepare(
      `UPDATE stack_candidate_sources SET trial_drops_sent = ?, trial_avg_rating = ? WHERE domain = ?`,
    ).run(s.count, s.avg, c.domain);
    if (s.count < cfg.trialDropsCount) continue;
    if ((s.avg ?? 0) >= cfg.promotionMinAvgRating)
      setStatus(db, c.domain, 'promoted', nowIso);
    else if ((s.avg ?? 10) <= cfg.archiveMaxAvgRating)
  ...
  for (const c of listCandidatesByStatus(db, 'promoted')) {
    const row = db
      ...
      SELECT COUNT(r.rating) AS c, AVG(r.rating) AS a
      FROM stack_queue q JOIN stack_ratings r ON r.drop_id = q.id
      WHERE q.source_url LIKE ?`,
      )
      .get(`%${c.domain}%`) as { c: number; a: number };
    if (
      (row?.c ?? 0) >= cfg.demotionMinRatedCount &&
      row.a < cfg.demotionAvgRatingThreshold
    ) {
      setStatus(db, c.domain, 'archived', nowIso);
    }
  }
