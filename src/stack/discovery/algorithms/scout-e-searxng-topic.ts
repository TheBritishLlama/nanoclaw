...
function topRatedTags(
  ...
  const rows = db
    ...
    .all(minRating, sinceIso) as { tags: string }[];
  ...
  async run(ctx: DiscoveryContext): Promise<CandidateSourceObservation[]> {
    ...
    for (const topic of topics) {
      for (const tmpl of QUERY_TEMPLATES) {
        ...
        try {
          results = await ctx.search.search(q);
        } catch {
          results = [];
        }
        queries++;
        ...
        for (const r of results) {
          ...
          upsertCandidateSource(ctx.db, {
            domain: d,
            origin_algorithm: 'scout_E_searxng_topic',
