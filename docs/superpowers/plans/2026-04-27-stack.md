...
**Architecture:** New NanoClaw feature skill on a `skill/stack` branch. Pipeline: 8 scrapers → Qwen 3 14B grader → Claude Haiku 4.5 enricher → confidence-gated SQLite queue (mirrored to Obsidian vault) → daily picker → Gmail outbound. Inbound handler parses numeric ratings (silent) and `/learn`/`/more` commands (one-line ack). Foundations track runs in parallel as a finite curriculum mixed 50/50 with discovered drops. Adaptive curator pipes recent ratings + free-form feedback back into the grader prompt.
...
│   ├── pipeline/
│   │   ├── grader.ts                             # Qwen 3 14B grading (batched)
...
- Qwen 3 14B grader (Ollama) + Claude Haiku 4.5 enricher
